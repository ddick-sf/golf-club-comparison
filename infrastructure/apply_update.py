...
final_rows = [r for r in orig if r['Manufacturer'] not in ['TaylorMade', 'Callaway', 'PING']]
final_rows = new_rows + final_rows

fieldnames = orig[0].keys()
...
import update_data  # Just to re-run the json generation
def generate_js():
    d_rows = list(csv.DictReader(open('Data/Golf Comparison - Drivers.csv', 'r', encoding='utf-8')))
    f_rows = list(csv.DictReader(open('Data/Golf Comparison - Fairway.csv', 'r', encoding='utf-8')))
    i_rows = list(csv.DictReader(open('Data/Golf Comparison - Irons.csv', 'r', encoding='utf-8')))
